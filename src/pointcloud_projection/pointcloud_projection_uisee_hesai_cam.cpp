...
void projectPointCloud(const sensor_msgs::PointCloud2::ConstPtr &pointCloudCur, cv::Mat &src_img, const nav_msgs::Odometry::ConstPtr &odometryCur)
{
    ...
    //! 提取相机视野下的点云
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloudInCameraView(new pcl::PointCloud<pcl::PointXYZI>());
    for (int i = 0; i < cloudIn.points.size(); ++i)
    {
        pcl::PointXYZI p = cloudIn.points[i];
        // if (p.y >= 0 && abs(p.x / p.y) <= 10 && abs(p.z / p.y) <= 10)
        if (p.y >= 0)
            cloudInCameraView->push_back(p);
    }
    cloudIn = *cloudInCameraView;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber subPointCloud = nh.subscribe<sensor_msgs::PointCloud2>("/all_cur_cloud", 1000, pointCloudCallback); // 点云
    ros::Subscriber subImage = nh.subscribe<sensor_msgs::Image>("/camera/image_raw", 1000, imageCallback);              // 图像
    ros::Subscriber subImu = nh.subscribe<sensor_msgs::Imu>("/imu/data", 1000, imuCallback);                            // IMU
