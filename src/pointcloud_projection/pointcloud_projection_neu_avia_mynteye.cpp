...
void projectPointCloud(const sensor_msgs::PointCloud2::ConstPtr &pointCloudCur, cv::Mat &src_img, const nav_msgs::Odometry::ConstPtr &odometryCur)
{
    ...
    cv::fisheye::estimateNewCameraMatrixForUndistortRectify(cameraMatrix, distCoeffs, imageSize, cv::Matx33d::eye(), optimalNewCameraMatrix, 1);
    ...
    //Step02. 计算map1, map2
    cv::fisheye::initUndistortRectifyMap(cameraMatrix, distCoeffs, cv::Matx33d::eye(), optimalNewCameraMatrix, imageSize, CV_16SC2, map1, map2);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber subPointCloud = nh.subscribe<sensor_msgs::PointCloud2>("/cloud_registered", 1000, pointCloudCallback);
    // ros::Subscriber subPointCloud = nh.subscribe<sensor_msgs::PointCloud2>("/livox_pcl0", 1000, pointCloudCallback);
    ros::Subscriber subImage = nh.subscribe<sensor_msgs::Image>("/mynteye/left/image_raw", 1000, imageCallback);
    ros::Subscriber subImu = nh.subscribe<sensor_msgs::Imu>("/imu", 1000, imuCallback);                  // IMU
    ros::Subscriber subOdometry = nh.subscribe<nav_msgs::Odometry>("/Odometry", 1000, odometryCallback); // 位姿
